...
var FluxCart = React.createClass({
 ...
 render: function() {
  ...
  return ( < div className = {
    "flux-cart " + (this.props.visible ? 'active' : '')
   } >
   ...
   onClick = {
    ...
    Object.keys(products).map(function(product) {
     return ( < li key = {
      ...
      onClick = {
       self.removeFromCart.bind(self, product)
      } > Remove < /button> < /li>
     )
    })
   } < /ul> < span className = "total" > Total: $ {
    this.props.total
   } < /span> < /div> < button type = "button"
   className = "view-cart"
   onClick = {
    this.openCart
   }
   disabled = {
    Object.keys(this.props.products).length > 0 ? "" : "disabled"
   } > View Cart({
    this.props.count
   }) < /button> < /div>
  );
